        ...
        return {
            ...
            link: function(scope, $elm,attr) {
                $elm.on('click', function() {
                    $(attr.value).animate({scrollTop: 0 }, "slow");
                });
            }
        }
        ...
        return{
            ...
            link: function(scope, ele, attrs) {
                attrs.$observe('value', function (param) {
                    ...
                    if(updateDate=='Invalid Date'){scope.styleselect[0]=param; return;}//若时间是一个无效日期，直接返回参数
                    ...
                    //时间的判断
                    if(difftime2<=1 && !type){updateDate="刚刚"}
                    else if(difftime2>1 && difftime2<=59 && !type){updateDate=difftime2+"秒前";}
                    else if(difftime2/60>=1 && difftime2/60<=59 && !type){updateDate=parseInt(difftime2/60)+"分钟前";}
                    else if(difftime2<difftime1){updateDate="今天"+(!type ? "":" ")+$filter('date')(updateDate,"HH:mm");}
                    else if(difftime2<(difftime1+3600*24)){ updateDate=(!type ? "昨天":"明天 ")+$filter('date')(updateDate,"HH:mm");}
                    else if(difftime2<(difftime1+3600*24*2)){updateDate=(!type ? "前天":"后天 ")+$filter('date')(updateDate,"HH:mm");}
                    else if(difftime2<(difftime1+3600*24*remainingDays)){updateDate="本周"+days[updateDate.getDay()]+(!type ? "":" ")+$filter('date')(updateDate,"HH:mm");}
                    else if(difftime2<(difftime1+3600*24*(7+remainingDays))){updateDate=(!type ? "上周":"下周")+days[updateDate.getDay()]+(!type ? "":" ")+$filter('date')(updateDate,"HH:mm");}
                    else if(updateDate.getFullYear()==currentDate.getFullYear()){updateDate=$filter('date')(updateDate,"MM-dd HH:mm");}
                    else{updateDate=$filter('date')(updateDate,"yyyy-MM-dd HH:mm");}
                    scope.mydate = updateDate;
                    if(!type){scope.styleselect[0]=scope.mydate;scope.styleselect[1]="";}
                    else{
                       if(scope.mydate.split(" ")[0].length<=3){scope.styleselect[0]=scope.mydate.split(" ")[0]+" ";scope.styleselect[1]=scope.mydate.split(" ")[1];}
                       else{scope.styleselect[0]="";scope.styleselect[1]=scope.mydate;}
                    }
                })
            }
        }
        ...
        return{
            ...
            link: function(scope, ele, attrs) {
                attrs.$observe('value', function (param) {
                    ...
                    param=param.split(",")[0];//需转换的时间
                    var updateDate=new Date(toDateTime(param));//更新时间的日期
                    if(updateDate=='Invalid Date'){scope.styleSelectDate[0]=param; return;}//若时间是一个无效日期，直接返回参数
                    var currentDate=new Date();
                    var days=['日','一','二','三','四','五','六'];
                    var updateTimeTimestamp=updateDate.getTime();//更新时间的时间戳
                    if(!type){//主题列表时间显示
                    ...
                    //时间的判断
                    if(difftime2<=1 && !type){updateDate="刚刚"}
                    else if(difftime2<difftime1){updateDate="今天"+(!type ? "":" ");}
                    else if(difftime2<(difftime1+3600*24)){ updateDate=(!type ? "昨天":"明天 ");}
                    else if(difftime2<(difftime1+3600*24*2)){updateDate=(!type ? "前天":"后天 ");}
                    else if(difftime2<(difftime1+3600*24*remainingDays)){updateDate="本周"+days[updateDate.getDay()]+(!type ? "":" ");}
                    else if(difftime2<(difftime1+3600*24*(7+remainingDays))){updateDate=(!type ? "上周":"下周")+days[updateDate.getDay()]+(!type ? "":" ");}
                    else if(updateDate.getFullYear()==currentDate.getFullYear()){updateDate=$filter('date')(updateDate,"MM-dd");}
                    else{updateDate=$filter('date')(updateDate,"yyyy-MM-dd");}
                    scope.mydate = updateDate;
                    if(!type){scope.styleSelectDate[0]=scope.mydate;scope.styleSelectDate[1]="";}
                    else{
                        if(scope.mydate.split(" ")[0].length<=3){scope.styleSelectDate[0]=scope.mydate.split(" ")[0]+" ";scope.styleSelectDate[1]=scope.mydate.split(" ")[1];}
                        else{scope.styleSelectDate[0]="";scope.styleSelectDate[1]=scope.mydate;console.log(scope.styleSelectDate[0],scope.styleSelectDate[1])}
                    }
                })
            }
        }
        ...
        return{
            ...
            link: function(scope, ele, attrs) {
                attrs.$observe('value', function (param) {
                    ...
                    param=param.split(",")[0];//需转换的时间
                    var updateDate=new Date(toDateTime(param));//更新时间的日期
                    if(updateDate=='Invalid Date'){scope.styleSelectOpenDate[0]=param; return;}//若时间是一个无效日期，直接返回参数
                    var currentDate=new Date();
                    var days=['日','一','二','三','四','五','六'];
                    var updateTimeTimestamp=updateDate.getTime();//更新时间的时间戳
                    if(!type){//主题列表时间显示
                    ...
                    scope.mydate = updateDate;
                    if(!type){scope.styleSelectOpenDate[0]=scope.mydate;scope.styleSelectOpenDate[1]="";}
                    else{
                        if(scope.mydate.split(" ")[0].length<=3){scope.styleSelectOpenDate[0]=scope.mydate.split(" ")[0]+" ";scope.styleSelectOpenDate[1]=scope.mydate.split(" ")[1];}
                        else{scope.styleSelectOpenDate[0]="";scope.styleSelectOpenDate[1]=scope.mydate;console.log(scope.styleSelectOpenDate[0],scope.styleSelectOpenDate[1])}
                    }
                })
            }
        }
