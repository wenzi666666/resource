(function(){
    ...
    UM.registerWidget('emotion',{
        ...
        sourceData: {
            emotion: {
                tabNum:7, //切换面板数量
                SmilmgName:{ 'edui-emotion-Jtab0':['', 50]}, //图片前缀名
        ...
        initContent:function( _editor, $widget ){
            ...
            var me = this,
                ...
                emotionUrl = UMEDITOR_CONFIG.UMEDITOR_HOME_URL + '/ewebeditor/sysimage/emot/',
                options = $.extend( {}, lang, {
            ...
            emotion.SmileyPath = _editor.options.emotionLocalization === false ? emotionUrl + '' : emotionUrl;
            emotion.SmileyBox = me.createTabList( emotion.tabNum );
            emotion.tabExist = me.createArr( emotion.tabNum );

            options['cover_img'] = emotion.SmileyPath + (editor.options.emotionLocalization ? '01.gif' : '01.gif');
            ...
            for ( ; i < imgNum; ) {
                ...
                for ( var j = 0; j < imgColNum; j++, i++ ) {
                    ...
                    if ( faceImage ) {
                        ...
                        var tmpIndex = ''
                        if ( i < 9 ) tmpIndex =  '0' + (i+1).toString()
                        else tmpIndex =   (i+1).toString()

                        textHTML.push( '<td  class="edui-emotion-' + tableCss + '" data-surl="'+ sUrl +'" data-realurl="'+ realUrl +'" data-posflag="'+ posflag +'" align="center">' );
                        textHTML.push( '<span>' );
                        textHTML.push( '<img  style="background-position:left ' + offset + 'px;" src="' + emotion.SmileyPath + (editor.options.emotionLocalization ? '01.gif" width="' : tmpIndex +'.gif" width="') + iWidth + '" height="' + iHeight + '"></img>' );
                        textHTML.push( '</span>' );
                    } else {
