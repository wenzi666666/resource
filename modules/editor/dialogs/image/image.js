(function () {
    ...
    var utils = UM.utils,
        ...
        getAllPic: function (sel, $w, editor) {
            ...
            $.each($imgs, function (index, node) {
                ...
                console.log(node)
                return arr.push({
                    // _src: node.src.split('?')[0],
                    src: node.src.split('?')[0],
                    type: 'imgUPload',
                    path: $(node).attr('path').replace('\/upFile', '')                                                                
                });
            });
        ...
        close: function ($img) {
            $img.css({
                ...
                if(Upload.showCount == 1){
                    $('.edui-image-custom').css('display', 'block')
                }
        ...
        callback: function (editor, $w, files, state) {
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                $('.edui-image-custom').hide();
                var $img = $("<img src='" + editor.options.imagePath + files[0] + "' type='uploadImg' class='edui-image-pic' path='"+ files[1] + "'/>"),
    ...
    var Upload = {
        ...
        uploadImgToCloud: function() {
            ...
            var uploader = new plupload.Uploader({
                ...
                filters: [{title: "图片文件", extensions: "jpg,gif,jpeg,png,bmp"}] //文件过滤
            });
            ...
            // 处理滚动条
            var $progress = $('#fsUploadProgress-img')
            ...
            uploader.bind('UploadComplete', function() {
                ...
                setTimeout(function(){
                    if (!!window.ActiveXObject || "ActiveXObject" in window) {
                        $('.edui-image-item img').addClass("thumbnail-img");
                     }
                },100)
            ...
            uploader.bind('FileUploaded', function(up, file, info) {
                ...
                Base.callback(me.editor, me.dialog, [window.FileServerUrl + response.uuid + fileType, response.save_path], 'SUCCESS');

                // Base.callback(me.editor, me.dialog, 'http://www.baidu.com/tmp.jpg', 'SUCCESS');
                // progress.setComplete(up, info);
                $("#fsUploadProgress-img").css("display", 'none')
            });
    ...
    var NetWork = {
        ...
        initEvt: function () {
            ...
            $(".edui-image-searchAdd", me.dialog).on("click", function (e) {
                e.stopPropagation();
    ...
    UM.registerWidget('image', {
        ...
        tpl: "<div id=\"container\" class=\"edui-image-wrapper\">" +
            ...
            "<br>&nbsp;&nbsp;图片：jpg、gif、jpeg、png、bmp，限制5M；</ul>" +
            "<div class=\"edui-tab-content\">" +
            "<div id=\"pickfiles-img\" class=\"edui-image-local edui-tab-pane edui-active\">" +
            "<div id=\"fsUploadProgress-img\"></div>" +
            "<div class=\"edui-image-content\"><div class=\"edui-image-custom\"></div></div>" +
